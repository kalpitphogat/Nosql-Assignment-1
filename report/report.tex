...
\item The server is shared with other users. It was fairly quiet when both
sessions started, but we cannot rule out that someone else used it during our
runs. The spread between runs is visible in Table~\ref{tab:totals} and in
Figure~\ref{fig:perquery}.
\item Some jumps have no explanation in the plans. Q2 (3.12), Q11 (3.28) and
Q21 (3.18) grew quickly from SF2 to SF4 with the same plan shape, and so did
Q19 at SF32. We report them without a cause.
...
\subsubsection*{Which stage takes the time}
...
than a second up to 500\,MB, because it only receives the roughly 45\,\% of
rows that pass the filter, the lines are short, and byte-wise comparison with
...
Environment unchanged across scale factors & met (shared server noted) \\
